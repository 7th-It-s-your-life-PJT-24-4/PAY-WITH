...
5. If the diff touches DB schema, MyBatis mappers, domain/DTO fields, or FE API types, verify the data shape is consistent end to end before running validation:
   - **migration ↔ mapper ↔ domain/DTO**: for every changed table/column in `be/src/main/resources/db/migration/`, confirm the corresponding `resultMap`/`#{...}` bindings in the mapper XML use the same column names, and that the Java `domain`/`dto` field types match the column types (e.g. `BIGINT` → `Long`, `VARCHAR` → `String`, nullable columns → boxed types). Do this by reading the changed migration, the mapper XML, and the domain/DTO class side by side — do not assume from naming alone.
   - **schema changes must ship as a migration**: if the diff adds or alters a table/column but contains no new `V<timestamp>__*.sql` under `be/src/main/resources/db/migration/`, flag it. Deployment applies schema only through Flyway, so a change made anywhere else reaches the app without reaching the database. Equally, flag any edit to `V1__baseline.sql` or to an already-merged migration — Flyway checksum validation will block startup.
