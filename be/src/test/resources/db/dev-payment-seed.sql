...
-- (2026-08-11: merchant_id 대역을 9001~9005 → 9901~9905 로 이전. 공용 R__seed.sql 에
--  제주권 시연 가맹점 9001~9003 이 신설돼 ID 가 겹쳤고, R__seed 는 ON DUPLICATE KEY UPDATE
--  라 재적용 때마다 서로의 값을 덮어썼다. merchant_id 는 users.user_id 와 별개 PK 공간이라
--  9001/9002 사용자 시드는 그대로 둔다.)
...
INSERT INTO merchants (merchant_id, name, category_code, region, latitude, longitude) VALUES
    (9901, '결제테스트 좌표미등록상점', 'MART', '서울 종로구', NULL, NULL)
...
INSERT INTO merchants (merchant_id, name, category_code, region, latitude, longitude) VALUES
    (9902, '결제테스트 금은방',     'JEWELRY',     '서울 종로구',   37.5700000, 126.9850000),
    (9903, '결제테스트 전자상가',   'ELECTRONICS', '서울 중구',     37.5670000, 126.9930000),
    (9904, '결제테스트 부산식당',   'RESTAURANT',  '부산 해운대구', 35.1587000, 129.1604000),
    (9905, '결제테스트 무분류상점', NULL,          '서울 종로구',   37.5750000, 126.9800000)
